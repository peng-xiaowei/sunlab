# Probit - log(dose) model（R 4.0 兼容版）
rm(list=ls())
library(MASS)
library(ggplot2)
library(effects)
...
# 读取数据
Data <- read.table("C:/Users/pxw/Desktop/JerseyCity-2.txt", header = TRUE, quote = "\"")
...
for (i in seq_along(trt.valid)) {
  ...
  # 初步拟合
  mod.1 <- glm(probability ~ log.dose, data = data.3, family = binomial(link = "probit"), weights = N)
  ...
  # 最终拟合
  mod <- glm(probability ~ log.dose, data = data.6, family = binomial(link = "probit"), weights = N)
  ...
  cat("LDπ and 95% CLs (probit model, Fieller's Theorem):\n")
  colnames(lethal.dose.f) <- c("π", "LDπ", "95%CL:lower", "95%CL:upper")
  print(round(lethal.dose.f, 3))
  
  cat("\nLDπ and 95% CLs (probit model, Delta Method):\n")
  ...
  if (nrow(data.7) > 0 && all(data.7$probability >= 0) && all(data.7$probability <= 1)) {
    ...
    p <- ggplot(data = data.7, aes(x = log.dose, y = probability)) +
      geom_point(size = 2) +
      geom_smooth(method = "glm", method.args = list(family = binomial(link = "probit")), aes(weight = N), colour = "red", se = TRUE) +
  ...
  for (i in 1:length(trt.valid)) {
    for (j in 1:length(trt.valid)) {
      if (!is.na(trt.valid[i]) && !is.na(trt.valid[j]) && trt.valid[i] != trt.valid[j]) {
        ...
        # 子集数据
        mod.i <- glm(probability ~ log.dose, family = binomial(link = "probit"), data = bind, weights = N, subset = (Trt.2 == trt.valid[i]))
        mod.j <- glm(probability ~ log.dose, family = binomial(link = "probit"), data = bind, weights = N, subset = (Trt.2 == trt.valid[j]))
...
mod.unconst <- glm(probability ~ Trt.2 + Trt.2:log.dose, data = bind.clean, 
                   family = binomial(link = "probit"), weights = N)
